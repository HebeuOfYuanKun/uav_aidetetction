...
namespace AVSAnalyzer {
    ...
    bool Scheduler::initAlgorithm() {
        ...
        bool ret = false;
        if (mConfig->algorithmEngine == "OpenVINO") {
            mAlgorithm = new AlgorithmOpenVINO(mConfig);
            ret = true;
        }else if(mConfig->algorithmEngine == "TensorRT") {
            mAlgorithm = new AlgorithmTensorRT(mConfig);
            ret = true;
        }
        else {
            LOGE("配置文件config.json 请输入正确的推理引擎参数: OpenVINO 或 TensorRT");
        }
        LOGI("初始化算法模型成功");
        return ret;
    }
    ...
    void Scheduler::apiControlAdd(Control* control, int& result_code, std::string& result_msg) {
        ...
        if (getWorkerSize() >= mConfig->workerConcurrency) {
    ...
    bool Scheduler::addWorker(Control* control, Worker* worker) {
        ...
        mWorkerMapMtx.lock();
        if (mWorkerMap.size() < mConfig->workerConcurrency) {
            bool isAdd = mWorkerMap.end() != mWorkerMap.find(control->code);
            if (!isAdd) {
