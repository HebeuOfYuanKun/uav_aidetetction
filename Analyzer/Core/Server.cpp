...
void Server::start(void* arg) {// 实现了一个服务器的启动逻辑，使用了 libevent 库进行 HTTP 服务器的创建，并且使用了 C++ 的多线程库 std::thread 来启动一个独立的线程运行事件循环。
    Scheduler* scheduler = (Scheduler*)arg;
    scheduler->setState(true);

    std::thread([](Scheduler *scheduler) {

        LOGI("启动视频分析器服务：http://0.0.0.0:%d", scheduler->getConfig()->analyzerPort);
