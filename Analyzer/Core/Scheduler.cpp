...
namespace AVSAnalyzer {
    ...
    {
        LOGI("");
        if (algorithm_onnx_yolo8) {
            delete algorithm_onnx_yolo8;
            algorithm_onnx_yolo8 = nullptr;
        }
        if (algorithm_tr_yolo8) {
            delete algorithm_tr_yolo8;
            algorithm_tr_yolo8 = nullptr;
        }
        if (algorithm_ov_yolo8) {
            delete algorithm_ov_yolo8;
            algorithm_ov_yolo8 = nullptr;
        }
    ...
    bool Scheduler::initAlgorithm() {
        LOGI("initAlgorithm() start");
        try {
            //onnx YOLOv8推理
            std::string onnxModelPath = mConfig->onnxRuntime;
            algorithm_onnx_yolo8 = new AlgorithmOnnxRuntime(mConfig);

            //TensorRt 推理
            std::string tensorrtModelPath = mConfig->tensorRt;
            algorithm_tr_yolo8 = new AlgorithmTensorRT(mConfig);

            //OpenVino 推理
            std::string modelPath = mConfig->openVINO;
            algorithm_ov_yolo8 = new AlgorithmOpenVINO(mConfig);
        }
        catch (ExecutionMode e) {

        }

        
        LOGI("initAlgorithm() end");
        return true;
    }
